...
@Service
public class CommentDataProviderImpl extends AbstractCommentDataProvider
        implements DataProvider<CommentModel> {

    @Override
    public Long getRowCount() {
        return commentDAO.getApprovedCommentsCount();
    }

    @Override
    public List<CommentModel> getItemsByRange(DataModel<CommentModel> dataModel,
            int first, int numberOfRows, Object... objects) {
        List<Comment> comments = commentDAO.getApprovedComments(first, numberOfRows);
